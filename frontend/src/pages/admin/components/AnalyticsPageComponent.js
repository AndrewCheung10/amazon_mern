...
const AnalyticsPageComponent = ({
    ...
    useEffect(() => {
        ...
        const handler = (newOrder) => {
            var orderDate = new Date(newOrder.createdAt).toLocaleString(
                "en-US",
            ...
            if (new Date(newOrder.createdAt).toDateString() === today) {
                if (today === new Date(firstDateToCompare).toDateString()) {
                    ...
                    today === new Date(secondDateToCompare).toDateString()
                ) {
        ...
        fetchOrdersForFirstDate(abctrl, firstDateToCompare)
            .then((data) => {
                ...
                const orders = data.map((order) => {
                    ...
                    var date = new Date(order.createdAt).toLocaleString(
                        "en-US",
                        {
                            hour: "numeric",
                            hour12: true,
